    ...
    process {
        ...
        [System.Enum]::GetNames([ExecutionSource]) |
            ForEach-Object {
                ...
                & "$PSScriptRoot/execute-task.ps1" -Message "Invoking $Stage executions from the $_ source" -Script {
                    $sourceExecutions |
                        ForEach-Object {
...
& "$PSScriptRoot/execute-task.ps1" -Message "Picking '$commitMessage'" -Script {
    git cherry-pick --no-commit $commitId
}

Invoke-Execution -Stage ([ApplicationStage]::Pick) -Execution $executions

& "$PSScriptRoot/execute-task.ps1" -Message "Replacing tokens" -Script {
...
& "$PSScriptRoot/execute-task.ps1" -Message "Adding changes" -Script {
    git add .
}

Invoke-Execution -Stage ([ApplicationStage]::Add) -Execution $executions

& "$PSScriptRoot/execute-task.ps1" -Message "Committing '$commitMessage'" -Script {
