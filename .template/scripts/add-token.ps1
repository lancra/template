...
$specification = Get-Content -Path $Specification |
    ConvertFrom-Json
...
$currentKeys = @($specification.tokens.PSObject.Properties) |
...
$newSpecification = [pscustomobject]@{
...
$specification.tokens.PSObject.Properties |
    ...
    Sort-Object |
    ForEach-Object {
        $newSpecification.tokens |
            Add-Member -MemberType NoteProperty -Name $_ -Value $specification.tokens.$_
    }
