...
& "$PSScriptRoot/execute-task.ps1" -Message "Fetching notes for template repository" -Script {
    git -C $Repository fetch origin refs/notes/*:refs/notes/*
}

& "$PSScriptRoot/execute-task.ps1" -Message "Creating local template branches for '$Template'" -Script {
...
& "$PSScriptRoot/execute-task.ps1" -Message "Switching to target template branch" -Script {
    git -C $Repository switch "$Template"
}

& "$PSScriptRoot/execute-task.ps1" -Message "Adding template remote to current repository" -Script {
    git remote add template $Repository
    git remote update template
}


& "$PSScriptRoot/execute-task.ps1" -Message "Fetching notes for template remote" -Script {
    git fetch template refs/notes/*:refs/notes/*
}


& "$PSScriptRoot/execute-task.ps1" -Message "Disabling reuse of recorded resolutions in repository" -Script {
    git config set --local rerere.enabled false
}

& "$PSScriptRoot/execute-task.ps1" -Message "Extracting template specification to '$Specification'" -Script {
    git show "template/${Template}:.template/specification.json" > $Specification
}

$todoFile = '.template.todo'

& "$PSScriptRoot/execute-task.ps1" -Message "Adding template TODO to local ignore file" -Script {
    Add-Content -Path '.git/info/exclude' -Value "$todoFile`n" -NoNewline
}

& "$PSScriptRoot/execute-task.ps1" -Message "Creating template TODO" -Script {
